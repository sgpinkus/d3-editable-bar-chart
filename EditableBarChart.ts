...
export function draw(container: SVGSVGElement, _options: Partial<Options> = {}, data: number[], cb: any) {

  function started(this: Element, event: DragEvent) {
  ...
  function getRange() {
    ...
    const [min, max] = [rangeMin ?? Math.min(...data), rangeMax ?? Math.max(...data)];
  ...
  let selected = [-1,-1];
  const xPos = d3.scaleBand<number>().rangeRound([0, width]).padding(0.1).domain(Object.keys(data).map(i => +i));
  ...
  const dg = d3.drag<SVGGElement, number>();
